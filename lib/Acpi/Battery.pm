...
sub batteryOnLine{
	my($self) = shift;
	my($file) = "/sys/class/power_supply/ADP1/uevent";

	my $adaptor = Acpi::Adaptor->new( file => $file );
	my $power_supply_online = $adaptor->online;
...
{
	my($self) = shift;
	my $batts = Acpi::Batteries->new()->batteries;
